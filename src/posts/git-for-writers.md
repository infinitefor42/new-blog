...
很多人第一次听到 Git，都是在程序员嘴里。于是很容易把它和黑色终端、代码仓库、GitHub 绑定在一起，觉得这套东西离日常写作太远。

但真正写过长文章、死磕过大报告，或者被毕业论文折磨过的人都知道，最让人崩溃的时刻通常出现在第七轮修改之后——桌面上堆满了《年底总结-定稿》《年底总结-定稿-领导意见后》《小说大纲-修改版》《第二章-真的最终版》这类文件。

其实这就是在做版本管理，只是方式非常原始。
...
一份资料只要需要反复修改、需要应对外部反馈、怕改坏、还想回头看旧版本，就已经很适合接入 Git。长文章、读书笔记、小说草稿、项目方案，还有最折磨人的论文，全都是最典型的例子。
...
## 三、为什么推荐 Markdown、Typst、LaTeX 这类文本格式

Git 最擅长管理的，是**纯文本或结构化文本**。

如果写作资料能尽量往文本格式靠，版本管理就会顺手很多：

* 改了什么，一眼能看出来
* 哪一段删了、哪一段加了，`diff` 会直接告诉我们
* 回退时不会带上一堆看不见的格式噪音
* 多个版本放在一起，比较和整理起来也更清楚

把常见格式放在一起看，大概是这样：

| 格式 | 更适合什么场景 | 为什么适合接入 Git |
| :--- | :--- | :--- |
| **Markdown** | 文章大纲、章节草稿、研究笔记、资料整理 | 轻、简单、差异清楚，最容易开始 |
| **Typst** | 想认真排版但不想语法学太重的长篇写作 | 纯文本，排版能力强，阅读体验更现代 |
| **LaTeX** | 公式多、引用多、模板要求严的学术论文 | 学术生态成熟，天然适合版本管理 |
| **Word** | 反馈流转、最终必须交 docx 的场景 | 也能纳入 Git，但差异不如纯文本直观 |

不用觉得"所有文稿都必须换成 Markdown"。更实际的做法通常有两种：

* 如果甲方或学校最后必须交 docx，也完全可以把大纲、章节草稿、研究笔记和修改记录纳入纯文本工作流。
* 如果本来就准备走 Typst 或 LaTeX，那 Git 几乎就是天然搭档。

很多时候，值得接入 Git 的不只是最终的正文，而是**整套写作资料**：
...
把 Markdown 转换成 Word 早就不是难题。不管是用 Typora、Obsidian 这类写作软件导出，还是用 Pandoc 转换，基本都是点一下鼠标的事：
